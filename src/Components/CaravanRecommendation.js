  ...
  return (
    <div style={{ textAlign: "center", margin: "20px" }}>
      <Container className="carousel">
        <h5 style={{ textAlign: "center", marginLeft: "2%", fontWeight: "bold" }}>
          비가 올 가능성이 있어요! 카라반 여행은 어떠세요?
        </h5>
        <br />
        {campingData && campingData.length > 0 ? (
          <StyledSlider {...settings}>
            {campingData.map((camping, index) => (
              <div
                key={index}
                onClick={() => onNavigate(`/ProductDetail/${camping.contentId}`, { state: { camping } })}
              >
                <Card style={{ border: "none", width: "80%", margin: "0 auto" }}>
                  <Card.Img
                    variant="top"
                    src={camping.firstImageUrl ? camping.firstImageUrl : defaultImage}
                    style={{ height: "50%", objectFit: "cover", borderRadius: "0" }}
                    alt="캠핑장 사진입니다."
                  />
                  <Card.Body>
                    <Card.Title style={{ fontSize: "0.85rem", textAlign: "center", fontWeight: "bold" }}>
                      {camping.facltNm}
                    </Card.Title>
